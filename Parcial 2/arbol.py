...
def inorden_criaturas(arbol):
    if(arbol is not None):
        ...
        print('Criatura: ',arbol['info'],'Derrotado por: ',arbol['datos']['Derrotado por'])
        inorden_criaturas(arbol['der'])

def inorden_criaturas_completo(arbol):
    if(arbol is not None):
        inorden_criaturas_completo(arbol['izq'])
        print(arbol['info'],arbol['datos'])
        inorden_criaturas_completo(arbol['der'])

def inorden_dic(arbol,dic):
    if(arbol is not None):
        inorden_dic(arbol['izq'],dic)
        nombreDios=arbol['datos']['Derrotado por']
        cantidadDerrotados=dic.get(nombreDios)
        if(cantidadDerrotados is not None):
            cantidadDerrotados += 1
        else:
            cantidadDerrotados = 1
        dic.update({nombreDios:cantidadDerrotados})
        inorden_dic(arbol['der'],dic)
...
def inorden_contiene(arbol, valor):
    if(arbol is not None):
        inorden_contiene(arbol['izq'], valor)
        if valor in arbol['info']:
            print(arbol['info'],arbol['datos'])
        inorden_contiene(arbol['der'], valor)

def inorden_mostrar_datos(arbol, valor):
    if(arbol is not None):
        inorden_mostrar_datos(arbol['izq'], valor)
        if valor in arbol['info']:
            print(arbol['info'],arbol['datos'])
        inorden_mostrar_datos(arbol['der'], valor)


def mostrar_datos_star_wars(arbol, valor,altura_total=None):
    if(arbol is not None):
        mostrar_datos_star_wars(arbol['izq'], valor)
        if valor in arbol['info']:
            print(arbol['info'],arbol['datos'],'su altura en el nodo es: ',arbol['altura'])
            if arbol['altura'] == 0:
                print('El tipo de nodo es una hoja')
            elif arbol['altura'] == altura_total:
                print('El tipo de nodo es una Raiz')
            else:
                print('El tipo de nodo es intermedia')

        mostrar_datos_star_wars(arbol['der'], valor)

def inorden_metros_star_wars(arbol):
    if(arbol is not None):
        inorden_metros_star_wars(arbol['izq'])
        if arbol['datos'] and arbol['datos']['altura'] < 90:
            print(arbol['info'])
        inorden_metros_star_wars(arbol['der'])

def inorden_peso_star_wars(arbol):
    if(arbol is not None):
        inorden_peso_star_wars(arbol['izq'])
        if arbol['datos'] and arbol['datos']['peso'] > 75:
            print(arbol['info'])
        inorden_peso_star_wars(arbol['der'])
...
def inorden_mostrar_criaturas_derrocadas_por(arbol, valor):
    if(arbol is not None):
        inorden_mostrar_criaturas_derrocadas_por(arbol['izq'], valor)
        if valor in arbol['datos']['Derrotado por'] :
            print(arbol['info'],arbol['datos'])
        inorden_mostrar_criaturas_derrocadas_por(arbol['der'], valor)

def inorden_capturadas_por(arbol, valor):
    if(arbol is not None):
        inorden_capturadas_por(arbol['izq'], valor)
        if arbol['datos']['Capturada'] is not None and valor in arbol['datos']['Capturada'] :
             print(arbol['info'],arbol['datos'])
        inorden_capturadas_por(arbol['der'], valor)
